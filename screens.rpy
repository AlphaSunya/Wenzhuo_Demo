...
## 覆盖滑条颜色为暖金色，绕过蓝色PNG图像文件
style slider:
    ysize gui.slider_size
    left_bar Frame(Solid("#e8c87a"), gui.slider_borders)
    right_bar Frame(Solid("#4a3510"), gui.slider_borders)
    thumb Frame(Solid("#e8c87a"), Borders(4, 4, 4, 4))
...
screen main_menu():
    ...
    # 主界面BGM+火焰环境音
    on "show" action [Play("music", "audio/bgm_main.ogg", fadein=2.0),
                      Play("sound", "audio/sfx_fire.ogg", loop=True)]
    on "replace" action [Play("music", "audio/bgm_main.ogg", fadein=2.0),
                         Play("sound", "audio/sfx_fire.ogg", loop=True)]

    # 底层：动态视频循环播放
    add Movie(play="cover_moving_noaudio.webm", loop=True, size=(1280, 720)):
        xalign 0.15
        yalign 0.5
    ...
    # 中层：卷轴图叠加（透明底PNG）
    add "cover_scroll_claude_black_2.png":
        xalign 0.0
        #xoffset -150
        yalign 0.5
        ysize 1080
    ...
    vbox:
        ...
        add "#c8a86066" xsize 200 ysize 1 xalign 0.5
            ...
            vbox:
                ...
                if message == "Are you sure you want to quit?":
                    ...
                    label "此番归去，未竟之事便付东流。":
                        style "confirm_prompt"
                        xalign 0.5
                elif "Loading will lose" in message:
                    label "此举将覆去路，当真如此？":
                        style "confirm_prompt"
                        xalign 0.5
                elif "overwrite your save" in message:
                    label "此位存档将烟消云散，当真覆写？":
                        style "confirm_prompt"
                        xalign 0.5
                else:
                    label _(message):
...
style confirm_frame:
    background Frame(Solid("#000000ee"), gui.confirm_frame_borders)
...
################################################################################
# 开场题字屏幕——竖排双列，右列先出，左列后出
################################################################################
screen title_poem_vertical():
    zorder 100
